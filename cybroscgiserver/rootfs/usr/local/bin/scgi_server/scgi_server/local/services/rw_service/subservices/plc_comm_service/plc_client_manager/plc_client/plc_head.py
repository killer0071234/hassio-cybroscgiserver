...
from scgi_server.local.services.rw_service.subservices.plc_comm_service.plc_client_manager.plc_client.util import cybro_timestamp_to_datetime
...
def bytes_to_plc_head(data_bytes):
    ...
    file_system_info_bytes = \
        data_bytes[FILE_SYSTEM_INFO_OFFSET:
                   FILE_SYSTEM_INFO_OFFSET + FILE_SYSTEM_INFO_SIZE]

    scan_overrun_bytes = data_bytes[SCAN_OVERRUN_OFFSET:
                                    SCAN_OVERRUN_OFFSET + SCAN_OVERRUN_SIZE]

    head_values = _struct_head.unpack(head_bytes)
    file_system_values = _struct_file_system_info.unpack(
        file_system_info_bytes
    )
