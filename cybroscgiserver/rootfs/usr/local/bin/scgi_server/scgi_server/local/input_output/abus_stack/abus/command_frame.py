...
class CommandFrame:
    ...
    def __str__(self):
        ...
        body_str = f"{self.body_bytes.hex()}" \
...
class CommandFrameUtil:
    ...
    @classmethod
    def create_ping(cls) -> CommandFrame:
        return cls._create_request_with_command(Command.PING)

    @classmethod
    def create_autodetect_response(cls) -> CommandFrame:
    ...
    @classmethod
    def create_read_status(cls) -> CommandFrame:
        return cls._create_request_with_command(Command.READ_STATUS)

    @classmethod
    def create_read_code_memory_block(cls,
                                      segment_number: int,
                                      block_size: int) -> CommandFrame:
    ...
    @classmethod
    def create_write_random_memory(cls,
                                   one_byte_addr: list[int],
                                   two_byte_addr: list[int],
                                   four_byte_addr: list[int],
                                   one_byte_values: bytes,
                                   two_byte_values: bytes,
                                   four_byte_values: bytes) -> CommandFrame:
        counts_bytes = struct.pack(
            "<3H",
            len(one_byte_addr),
            len(two_byte_addr),
        ...
        addresses = one_byte_addr + two_byte_addr + four_byte_addr
        addresses_bytes = struct.pack(f"<{len(addresses)}H",
                                      *addresses)
    ...
    @classmethod
    def _create_request_with_command(cls,
                                     command: Command,
                                     body_rest: bytes = b"") -> CommandFrame:
        return cls._create_with_command(
            Direction.REQ, Type.COMMAND, command, body_rest
        )

    @classmethod
    def _create_response_with_command(cls,
                                      command: Command,
                                      body_rest: bytes = b"") -> CommandFrame:
        return cls._create_with_command(
            Direction.ACK, Type.COMMAND, command, body_rest
        )

    @classmethod
    def _create_with_command(cls,
                             msg_direction: Direction,
                             msg_type: Type,
                             command: Command,
                             body_rest: bytes) -> CommandFrame:
        body_bytes = struct.pack("<B", command.value) + body_rest
        return CommandFrame(msg_direction, msg_type.value, body_bytes)

    @classmethod
    def command_frame_to_bytes(cls, command_frame: CommandFrame) -> bytes:
    ...
    @classmethod
    def bytes_to_command_frame(cls,
                               command_frame_bytes: bytes) -> CommandFrame:
