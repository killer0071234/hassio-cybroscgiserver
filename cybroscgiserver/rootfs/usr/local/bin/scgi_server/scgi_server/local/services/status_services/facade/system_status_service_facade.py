...
class SystemStatusServiceFacade:
    ...
    def __init__(self, system_status_service: SystemStatusService):
        ...
        self._actions: dict[
            ...
            "abus_list": (
                self._abus_list,
                "Formated list of controllers with the current status."
            )
        }
    ...
    async def _push_list(self) -> str:
        ...
        headers = ["push", "nad", "ip address", "status", "program", "alc file", "downloaded", "response", "last update"]
        data = []
    ...
    async def _abus_list(self) -> str:
        column_width = [8, 12, 10, 20, 18, 8, 8]
        headers = ["nad", "messages", "errors", "last error", "ip address", "port", "origin"]
        data = []
