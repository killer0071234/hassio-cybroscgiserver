...
from scgi_server.local.services.rw_service.scgi_communication.operation_type import \
    OperationType
...
class RWRequest:
    ...
    @classmethod
    def _determine_target_tag_name_and_nad(
        cls,
        name: str
    ) -> Tuple[Target, str, Optional[int]]:
        ...
        if match is not None:
            ...
            if rest_of_name.startswith("sys"):
                return (cls.Target.PLC_SYSTEM,
                        rest_of_name.partition(".")[-1],
                        nad)
            else:
                return (cls.Target.PLC,
                        rest_of_name,
                        nad)
        else:
            raise InvalidTagNameError(name)
